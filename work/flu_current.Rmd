...
This R markdown report contains general summaries of questions by demographics.

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = F, warning = T, message = F)
...
## PPGENDER
# oddsratio with reversed columns
a <- with(data2, table(PPGENDER, Q1))
oddsratio(a, rev = "neither")
...
## ppagecat
a = with(data2, table(ppagecat, Q1))
oddsratio(a, rev = "columns")


## PPEDUCAT
xtab(PPEDUCAT ~ Q1, data2)
g1 + geom_bar(mapping = aes(x = PPEDUCAT, fill = Q1), position = position_stack())
g1 + geom_bar(mapping = aes(x = PPEDUCAT, fill = Q1), position = position_fill())
